...
This test suite demonstrates various pytest features and testing best practices:
...
def temp_city():
    """
    Pytest fixture that provides a clean copy of sample city data for each test.
...
def test_create_invalid_input_raises_error():
    """
    Test that create() properly validates input types and raises ValueError for invalid data.
    ...
    # Act & Assert: Call the function and verify it raises the expected exception
    # pytest.raises() context manager catches the exception and verifies its type
...
def test_success_create_increases_city_count():
    """
    Test the happy path for city creation - verify successful creation behavior.
...
def test_num_cities():
    ...
    This test is more specific than test_success_create_increases_city_count
    because it verifies the exact increment (by 1) rather than just "increased".
    """
